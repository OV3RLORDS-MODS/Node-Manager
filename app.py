...
class MainApp(tk.Tk):
    ...
    def create_tabs(self):
        ...
        self.mod_manager_tab = Mods(self.notebook, self.steam_api_key)  # This will still require the key if used
        self.player_management_tab = PlayerManagement(self.notebook)
        self.discord_tools_tab = DiscordTools(self.notebook)
        self.info_tab = Info(self.notebook)  # Correctly instantiate the Info class
        ...
        self.notebook.add(self.info_tab, text="Info")  # Add the Info tab
        self.notebook.add(self.game_settings_tab, text="Game Settings")
